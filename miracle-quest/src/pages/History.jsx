...
const History = () => {
  ...
  const getFilteredEntries = useCallback(() => {
    ...
    if (dateRange === "single") {
      ...
      return foodHistory.filter(
        (entry) => entry.date >= selectedDate && entry.date <= endDate
      );
    }
  ...
  const getDaysInRange = useCallback(() => {
    ...
    return diffDays + 1;
  }, [selectedDate, endDate, dateRange]);
  ...
  useEffect(() => {
    ...
    const averages = {
      calories: Math.round(totals.calories / daysInRange),
      carbohydrates: Math.round(totals.carbohydrates / daysInRange),
      protein: Math.round(totals.protein / daysInRange),
      fats: Math.round(totals.fats / daysInRange),
    };

    setNutritionSummary(averages);
  }, [getFilteredEntries, getDaysInRange]);

  // Add this before the return statement
  const currentItems = getFilteredEntries().slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Update local state
      const updatedHistory = foodHistory.filter(
        (item) =>
          !(
      ...
      const newSummary = updatedHistory.reduce(
        (acc, entry) => ({
          calories: acc.calories + entry.nutrition.calories,
          carbohydrates: acc.carbohydrates + entry.nutrition.carbs,
          protein: acc.protein + entry.nutrition.protein,
        ...
        <Card className="shadow-sm">
          <Card.Body>
            {foodHistory.length === 0 ? (
              <Alert variant="info">
                No food entries found. Start logging your meals to see them
                here!
              </Alert>
            ) : (
              <>
        ...
        <Modal.Body>
          {deletingEntry && (
            <p>
              ...
              <br />
              <br />
              <strong>Date:</strong> {formatDate(deletingEntry.date)}
              <br />
              <strong>Meal:</strong> {deletingEntry.mealTime}
              <br />
